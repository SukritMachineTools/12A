...
void setup() {
  ...
  WiFi.mode(WIFI_STA); // Change to station mode to connect to another WiFi network
  WiFi.config(staticIp, gateway, subnet); // Set static IP configuration
  WiFi.begin(ssid, password); // Connect to the new WiFi network
...

{
  if (digitalRead(optoPinm1) == LOW) {
    ...
    if (int(rtc.getHour()) < 10)
    {
      if (int(rtc.getMinute()) < 10)
      {
        myvar2 = "0" + String(rtc.getHour()) + ":0" + String(rtc.getMinute()) + ":" + String(rtc.getSecond());
      }
      else
      {
        myvar2 = "0" + String(rtc.getHour()) + ":" + String(rtc.getMinute()) + ":" + String(rtc.getSecond());
      }
    }
    else
    {
      if (int(rtc.getMinute()) < 10)
      {
        myvar2 = String(rtc.getHour()) + ":0" + String(rtc.getMinute()) + ":" + String(rtc.getSecond());
      }
      else
      {
        myvar2 = String(rtc.getHour()) + ":" + String(rtc.getMinute()) + ":" + String(rtc.getSecond());
      }
    }
  ...
  else if (digitalRead(optoPinm1) == HIGH && digitalRead(optoPinm2) == LOW) 
  {
    mode = 1;
    lcd.setCursor(17, 3);
    lcd.print(mode);
    ...
    if (millis() - sTime > 1000 && srunning == true) 
    {
      smin += 1;
      sTime = millis();
    }
    ...
    if (digitalRead(optoPinm3) == LOW && sflag == 1) 
    {
      scycleStartTime = millis();
      sflag = 0;
    }

    if (digitalRead(optoPinm3) == HIGH && sflag == 0) 
    {
      scycleTime = millis() - scycleStartTime;
      scycleTime = scycleTime / 1000;
      cycleTimeval = scycleTime;
      sflag = 1;
      EEPROM.put(scycleadd, smin);  // Write the integer to the first four slots
    }
  }

  else if (digitalRead(optoPinm2) == HIGH && digitalRead(optoPinm1) == HIGH && digitalRead(optoPinm3) == HIGH && digitalRead(optoPinm4) == HIGH && digitalRead(optoPinm5) == HIGH) 
  {
    if (milliFlag == 1) 
    {
    ...
    if (digitalRead(optoPinm3) == LOW && mflag == 1) 
    {
      mcycleStartTime = millis();
      mflag = 0;
    }

    if (digitalRead(optoPinm3) == HIGH && mflag == 0) 
    {
      mcycleTime = millis() - mcycleStartTime;
      mcycleTime = mcycleTime / 1000;
      cycleTimeval = mcycleTime;
      mflag = 1;
      EEPROM.put(mcycleadd, mmin);  // Write the integer to the first four slots
    }
  }
